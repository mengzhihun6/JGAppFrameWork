...
@interface JGHomeController () <UITableViewDataSource,UITableViewDelegate,SDCycleScrollViewDelegate>
{
    NSArray *dataSource;
}
...
@implementation JGHomeController
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    if (self.navColor) {
        ...
        [self.navigationController.navigationBar jg_setBackgroundColor:[UIColor clearColor] isHiddenBottomBlackLine:YES];
    }
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
    if (scrollView==self.table) {
        ...
        if (offsetY >0) {
            ...
            //            NSLog(@"alpha =%f",alpha);
            [self.navigationController.navigationBar jg_setBackgroundColor:[kTintColor colorWithAlphaComponent:alpha] isHiddenBottomBlackLine:YES];
            if (alpha>=0.99) {
